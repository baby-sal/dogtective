...
def get_all_desserts_front_end():
...
def add_new_menu_item(dessert_id, name, description, price):
...
# def function()
def main():
    print("Welcome to the Dessert Menu API!, Choose an option:")
    print("A: View all dessert menu items")
    print("B: Remove a dessert by ID")
    print("C: Add a dessert to the menu items")

    answer = input("What would you like to do? (A, B or C): ").strip().upper()

    if answer == "A":
        if get_all_desserts_front_end() is None:
            print("Failed to retrieve records.")

        print(get_all_desserts_front_end())
    elif answer == "B":
        characters = get_all_desserts_front_end()
        ...
        if characters is not None:
            ...
            print("Here is the registry:")
            print(delete_dessert_by_id(id_to_remove))
        else:
            print("Could not load characters for deletion.")
    elif answer == "C":
        dessert_id = int(input('a number over 3: '))
        name = input('add a dessert you would like to see on the menu')
        description = input('add a description for what you want in the dessert')
        price = float(input('enter a decimal price'))
        add_new_menu_item(dessert_id, name, description, price)
